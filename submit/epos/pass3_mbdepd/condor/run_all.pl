...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_BBC_G4HIT' and filename like 'DST_BBC_G4HIT_epos_0_153fm_50kHz_bkg_0_153fm%' and runnumber = $inrunnumber order by filename") || die $DBI::errstr;
my $chkfile = $dbh->prepare("select lfn from files where lfn=?") || die $DBI::errstr;

my $gettruthfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRUTH_G4HIT' and filename like 'DST_TRUTH_G4HIT_epos_0_153fm_50kHz_bkg_0_153fm%' and runnumber = $inrunnumber");
...
foreach my $segment (sort { $a <=> $b } keys %mbdhash)
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_MBD_EPD_epos_0_153fm_50kHz_bkg_0_153fm-%010d-%06d.root",$outrunnumber,$segment);
	if ($segment < 100000)
	{
	    $outfilename = sprintf("DST_MBD_EPD_epos_0_153fm_50kHz_bkg_0_153fm-%010d-%05d.root",$outrunnumber,$segment);
	}

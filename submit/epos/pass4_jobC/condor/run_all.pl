...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRACKSEEDS' and filename like 'DST_TRACKSEEDS_epos_0_153fm_50kHz_bkg_0_153fm%' and runnumber = $inrunnumber order by filename") || die $DBI::errstr;
my $chkfile = $dbh->prepare("select lfn from files where lfn=?") || die $DBI::errstr;

my $getclusterfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_CALO_CLUSTER' and filename like 'DST_CALO_CLUSTER_epos_0_153fm_50kHz_bkg_0_153fm%' and runnumber = $inrunnumber");
...
foreach my $segment (sort keys %trkhash)
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_TRACKS_epos_0_153fm_50kHz_bkg_0_153fm-%010d-%06d.root",$outrunnumber,$segment);
	if ($segment < 100000)
	{
	    $outfilename = sprintf("DST_TRACKS_epos_0_153fm_50kHz_bkg_0_153fm-%010d-%05d.root",$outrunnumber,$segment);
	}

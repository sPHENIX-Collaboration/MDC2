...
my $getfiles = $dbh->prepare("select filename from datasets where dsttype = 'G4Hits' and filename like '%epos_0_153fm%' and runnumber = $runnumber order by filename") || die $DBI::errstr;
my $chkfile = $dbh->prepare("select lfn from files where lfn=?") || die $DBI::errstr;

my $getbkglastsegment = $dbh->prepare("select max(segment) from datasets where dsttype = 'G4Hits' and filename like '%epos_0_153fm%' and runnumber = $runnumber");
...
# output file does not exist yet, check for 2 MB background files (n to n+1)
	...
	for (my $cnt = $segment+1; $cnt <=$segment+4; $cnt++)
	{
	    my $bkgseg = $cnt;

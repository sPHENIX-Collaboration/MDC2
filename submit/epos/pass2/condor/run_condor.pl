...
if ($#ARGV < 4)
{
    print "usage: run_condor.pl <nevents> <infile> <outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass2_epos.sh",$rundir);
my $nevents = $ARGV[0];
my $infile = $ARGV[1];
...
my $condorlogdir = sprintf("/tmp/epos/pass2/run%d",$runnumber);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRKR_HIT' and filename like '%Herwig_$jettriggerWithDash%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
if ($verbosity > 0)
{
    print "select filename,segment from datasets where dsttype = 'DST_TRKR_HIT' and filename like '%Herwig_$jettriggerWithDash%' and runnumber = $runnumber order by segment\n";
}
...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_TRKR_CLUSTER_Herwig_%s-%010d-%06d.root",$jettrigger,$runnumber,$segment);

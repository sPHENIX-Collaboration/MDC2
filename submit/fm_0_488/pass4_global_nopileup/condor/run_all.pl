...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRACKS' and filename like 'DST_TRACKS_sHijing_0_488fm-%' and runnumber = $inrunnumber order by filename") || die $DBI::errstr;
my $chkfile = $dbh->prepare("select lfn from files where lfn=?") || die $DBI::errstr;

my $getbbcepdfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_BBC_EPD' and filename like 'DST_BBC_EPD_sHijing_0_488fm-%' and runnumber = $inrunnumber");
...
foreach my $segment (sort keys %trkhash)
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_GLOBAL_sHijing_0_488fm-%010d-%06d.root",$outrunnumber,$segment);
	if ($segment < 100000)
	{
	    $outfilename = sprintf("DST_GLOBAL_sHijing_0_488fm-%010d-%05d.root",$outrunnumber,$segment);
	}

...
{
    $globalhash{sprintf("%05d",$res[1])} = $res[0];
}
$getfiles->finish();
...
foreach my $segment (sort keys %globalhash)
{
    ...
    my $lfn = $globalhash{$segment};
    push(@globalfiles,$lfn);
    ...
    my $outfilename = sprintf("DST_MDC2_GLOBAL_sHijing_0_20fm_50kHz_bkg_0_20fm-%010d-%05d.root",$outrunnumber,$outseg);
    my $vtxlistfile = sprintf("%s/condor-%010d-%05d.vtxlist",$logdir,$outrunnumber,$outseg);
    my $globallistfile = sprintf("%s/condor-%010d-%05d.globallist",$logdir,$outrunnumber,$outseg);
    open(F1,">$vtxlistfile");
    ...
    close(F1);
    open(F1,">$globallistfile");
    foreach my $bf (@globalfiles)
    {
	print F1 "$bf\n";
    }
    close(F1);
...
# create run and segment number to pass down
    ...
    my $subcmd = sprintf("perl run_condor.pl %d %d %d %s %s %s %s %d %d %s %s", $outevents, $outrunnumber, $outsegused, $outfilename, $outdir, $globallistfile, $vtxlistfile, $nrun, $nseg, $rawdatadir, $tstflag);

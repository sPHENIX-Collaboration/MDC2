...
my $executable = sprintf("%s/run_pass3trk_pau_0_10fm.sh",$rundir);
my $nevents = $ARGV[0];
my $infile0 = $ARGV[1];
my $infile1 = $ARGV[2];
my $dstoutdir = $ARGV[3];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/pAu_0_10fm/pass3trk/run%d",$runnumber);
mkpath($condorlogdir);
...
print F "Arguments       = \"$nevents $infile0 $infile1 $dstoutdir $runnumber $sequence\"\n";

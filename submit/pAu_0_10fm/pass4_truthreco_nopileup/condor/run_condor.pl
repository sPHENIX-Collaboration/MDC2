...
my $executable = sprintf("%s/run_pass4_truthreco_nopileup_pau_0_10fm.sh",$rundir);
my $nevents = $ARGV[0];
my $infile0 = $ARGV[1];
my $infile1 = $ARGV[2];
my $infile2 = $ARGV[3];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/pAu_0_10fm/pass4_truthreco_nopileup/run%d",$runnumber);
mkpath($condorlogdir);

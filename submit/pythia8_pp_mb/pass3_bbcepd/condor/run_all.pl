...
foreach my $segment (sort { $a <=> $b } keys %bbchash)
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_BBC_EPD_pythia8_pp_mb_3MHz-%010d-%06d.root",$outrunnumber,$segment);
	if ($segment < 100000)
	{
	    $outfilename = sprintf("DST_BBC_EPD_pythia8_pp_mb_3MHz-%010d-%05d.root",$outrunnumber,$segment);
	}

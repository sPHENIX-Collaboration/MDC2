...
my $executable = sprintf("%s/run_pass2calo_nopileup_nozero__pythia8_pp_mb.sh",$rundir);
my $nevents = $ARGV[0];
my $infile0 = $ARGV[1];
...
my $condorlogdir = sprintf("/tmp/pythia8_pp_mb/pass2calo_nopileup_nozero/run%d",$runnumber);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}
...
print F "Arguments       = \"$nevents $infile0 $dstoutfile $dstoutdir $runnumber $sequence\"\n";
...
print F "$executable, $nevents, $infile0,  $dstoutfile, $dstoutdir, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRUTH' and filename like '%$outfilestring%' and runnumber = $runnumber order by filename") || die $DBI::error;

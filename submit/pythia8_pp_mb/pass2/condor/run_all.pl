...
my $getfiles = $dbh->prepare("select filename from datasets where dsttype = 'G4Hits' and filename like '%pythia8_mb%' and runnumber = $runnumber order by filename") || die $DBI::error;

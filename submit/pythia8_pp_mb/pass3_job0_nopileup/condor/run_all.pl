...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRKR_HIT' and filename like '%pythia8_pp_mb%' and runnumber = $runnumber order by filename") || die $DBI::error;
...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_TRKR_CLUSTER_pythia8_pp_mb-%010d-%05d.root",$runnumber,$segment);

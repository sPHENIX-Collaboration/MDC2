...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRACKS' and filename like 'DST_TRACKS_pythia8_pp_mb_3MHz%' and runnumber = $inrunnumber order by filename") || die $DBI::errstr;
my $chkfile = $dbh->prepare("select lfn from files where lfn=?") || die $DBI::errstr;

my $getmbdepdfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_MBD_EPD' and filename like 'DST_MBD_EPD_pythia8_pp_mb_3MHz%' and runnumber = $inrunnumber");
...
foreach my $segment (sort keys %trkhash)
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_GLOBAL_pythia8_pp_mb_3MHz-%010d-%06d.root",$outrunnumber,$segment);
	if ($segment < 100000)
	{
	    $outfilename = sprintf("DST_GLOBAL_pythia8_pp_mb_3MHz-%010d-%05d.root",$outrunnumber,$segment);
	}

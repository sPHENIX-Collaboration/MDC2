...
my $executable = sprintf("%s/run_pileup_pythia8_pp_mb_1MHz.sh",$rundir);
my $nevents = $ARGV[0];
my $infile = $ARGV[1];
...
my $suffix = sprintf("1MHz-%010d-%05d",$runnumber,$sequence);
my $logdir = sprintf("%s/log/run%d",$localdir,$runnumber);
if (! -d $logdir)
{
  mkpath($logdir);
}
my $condorlogdir = sprintf("/tmp/pythia8_pp_mb/pass2_1MHz/run%d",$runnumber);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

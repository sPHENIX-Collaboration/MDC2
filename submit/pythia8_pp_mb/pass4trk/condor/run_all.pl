...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRKR_CLUSTER' and filename like '%pythia8_pp_mb_3MHz%' and runnumber = $runnumber order by filename") || die $DBI::error;

...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRKR_G4HIT' and filename like '%pythia8_mb%' and runnumber = $runnumber order by filename") || die $DBI::error;
my $chkfile = $dbh->prepare("select lfn from files where lfn=?") || die $DBI::error;
my $gettruthfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRUTH_G4HIT' and filename like '%pythia8_mb%'and runnumber = $runnumber");
my $nsubmit = 0;
...
foreach my $segment (sort keys %trkhash)
{
	...
	{
            my $lfn =  sprintf("%s_pythia8_mb-%010d-%05d.root",$type,$runnumber,$segment);
	    $chkfile->execute($lfn);
	    if ($chkfile->rows > 0)
	    {
		next;

...
if ($#ARGV < 5)
{
    print "usage: run_condor.pl <events> <trk hits> <outfile> <outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_job0_pythia8_pp_mb.sh",$rundir);
my $nevents = $ARGV[0];
my $infile = $ARGV[1];
...
my $condorlogdir = sprintf("/tmp/pythia8_pp_mb/pass4_job0/run%d",$runnumber);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

...
my $executable = sprintf("%s/run_pass4_truthreco_nopileup_pp_mb.sh",$rundir);
my $nevents = $ARGV[0];
my $infile0 = $ARGV[1];
my $infile1 = $ARGV[2];
my $infile2 = $ARGV[3];
...
my $condorlogdir = sprintf("/tmp/pythia8_pp_mb/pass4_truthreco_nopileup/run%d",$runnumber);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

...
if ($#ARGV < 6)
{
    print "usage: run_condor.pl <events> <mbd infile> <truth infile> <outfile> <outdir> <build> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass3_mbdepd_oo.sh",$rundir);
my $nevents = $ARGV[0];
my $pileup = $ARGV[1];
my $infile0 = $ARGV[2];
my $infile1 = $ARGV[3];
...
my $condorlogdir = sprintf("/tmp/OO_0_15fm/pass3_mbdepd/run%d/%s",$runnumber,$pileup);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}
...
print F "Arguments       = \"$nevents $infile0 $infile1 $dstoutfile $dstoutdir $build $runnumber $sequence\"\n";
...
print F "$executable, $nevents, $infile0 $infile1, $dstoutfile, $dstoutdir, $build, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio, $memory $batchname\n";
close(F);

...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRACKSEEDS' and filename like 'DST_TRACKSEEDS_sHijing_OO_0_15fm-%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
my $getclusterfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_CALO_CLUSTER' and filename like 'DST_CALO_CLUSTER_sHijing_OO_0_15fm-%' and runnumber = $runnumber");
...
foreach my $segment (sort keys %trkhash)
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_TRACKS_sHijing_OO_0_15fm-%010d-%06d.root",$runnumber,$segment);

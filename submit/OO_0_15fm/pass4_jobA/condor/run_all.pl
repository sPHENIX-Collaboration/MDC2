...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRKR_CLUSTER' and filename like 'DST_TRKR_CLUSTER_sHijing_OO_0_15fm_$pileupstring-%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_TRACKSEEDS_sHijing_OO_0_15fm_%s-%010d-%06d.root",$pileupstring,$runnumber,$segment);

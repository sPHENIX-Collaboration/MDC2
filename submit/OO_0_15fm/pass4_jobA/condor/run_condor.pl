...
if ($#ARGV < 7)
{
    print "usage: run_condor.pl <events> <pileup> <trk clusters> <outfile> <outdir> <build> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass4_jobA_oo.sh",$rundir);
my $nevents = $ARGV[0];
my $pileup = $ARGV[1];
my $infile = $ARGV[2];
...
my $condorlogdir = sprintf("/tmp/OO_0_15fm/pass4_jobA/run%d/%s",$runnumber,$pileup);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

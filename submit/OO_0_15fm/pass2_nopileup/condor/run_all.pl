...
my $getfiles = $dbh->prepare("select filename from datasets where dsttype = 'G4Hits' and filename like '%sHijing_OO_0_15fm%' and runnumber = $runnumber and segment >= $startsegment order by filename") || die $DBI::errstr;
...
#    print "found $lfn\n";
	...
	foreach my $type (sort keys %outfiletype)
	{
            my $lfn =  sprintf("%s_sHijing_OO_0_15fm-%010d-%06d.root",$type,$runnumber,$segment);
#            print "checking for $lfn\n";
	...
	my $calooutfilename = sprintf("DST_CALO_CLUSTER_sHijing_OO_0_15fm-%010d-%06d.root",$runnumber,$segment);
	my $globaloutfilename = sprintf("DST_MBD_EPD_sHijing_OO_0_15fm-%010d-%06d.root",$runnumber,$segment);

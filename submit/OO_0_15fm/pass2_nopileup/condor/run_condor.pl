...
my $executable = sprintf("%s/run_pass2_nopileup_oo_0_15fm.sh",$rundir);
my $nevents = $ARGV[0];
my $infile = $ARGV[1];
...
my $condorlogdir = sprintf("/tmp/OO_0_15fm/pass2_nopileup/run%d",$runnumber);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

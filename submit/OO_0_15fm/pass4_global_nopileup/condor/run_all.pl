...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRACKS' and filename like 'DST_TRACKS_sHijing_OO_0_15fm-%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
...
my $getmbdepdfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_MBD_EPD' and filename like 'DST_MBD_EPD_sHijing_OO_0_15fm-%' and runnumber = $runnumber");

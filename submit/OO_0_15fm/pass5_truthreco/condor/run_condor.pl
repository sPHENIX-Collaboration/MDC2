...
if ($#ARGV < 10)
{
    print "usage: run_condor.pl <events> <pileup> <trkr g4hit file> <trkr cluster file> <tracks file> <truth file> <outfile> <outdir> <build> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass5_truthreco_oo.sh",$rundir);
my $nevents = $ARGV[0];
my $pileup = $ARGV[1];
my $infile0 = $ARGV[2];
my $infile1 = $ARGV[3];
...
my $condorlogdir = sprintf("/tmp/OO_0_15fm/pass5_truthreco/run%d/%s",$runnumber,$pileup);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}
...
print F "Arguments       = \"$nevents $infile0 $infile1 $infile2 $infile3 $outfilename $dstoutdir $build $runnumber $sequence $pileup\"\n";
...
print F "$executable, $nevents, $infile0,  $infile1, $infile2, $infile3, $outfilename, $dstoutdir, $build, $runnumber, $sequence, $pileup, $outfile, $errfile, $condorlogfile, $rundir, $baseprio, $memory $batchname\n";
close(F);

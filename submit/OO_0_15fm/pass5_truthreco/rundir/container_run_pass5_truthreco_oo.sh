...
echo arg11 \(pileup\): ${11}
echo arg12 \(git commit id\): ${12}
echo cdbtag: $cdbtag

runnumber=$(printf "%010d" $9)
sequence=$(printf "%06d" ${10})

echo running root.exe -q -b Fun4All_TruthReco.C\($1,\"$2\",\"$3\",\"$4\",\"$5\",\"$6\",\"$7\",\"$cdbtag\",\"${12}\"\)
root.exe -q -b  Fun4All_TruthReco.C\($1,\"$2\",\"$3\",\"$4\",\"$5\",\"$6\",\"$7\",\"$cdbtag\",\"${12}\"\)

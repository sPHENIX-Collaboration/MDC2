...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_CALO_G4HIT' and filename like 'DST_CALO_G4HIT_sHijing_OO_0_15fm_$pileupstring-%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
if ($verbosity > 0)
{
  print "select filename,segment from datasets where dsttype = 'DST_CALO_G4HIT' and filename like 'DST_CALO_G4HIT_sHijing_OO_0_15fm_$pileupstring%-' and runnumber = $runnumber order by filename\n";
}
...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_CALO_CLUSTER_sHijing_OO_0_15fm_%s-%010d-%06d.root",$pileupstring,$runnumber,$segment);

...
my $outfilelike = sprintf("sHijing_OO_0_15fm_%s",$pileupstring);
...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRACKS' and filename like 'DST_TRACKS_$outfilelike-%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
#print "select filename,segment from datasets where dsttype = 'DST_TRACKS' and filename like 'DST_TRACKS_$outfilelike-%' and runnumber = $runnumber order by segment\n";
my $chkfile = $dbh->prepare("select lfn from files where lfn=?") || die $DBI::errstr;

my $getmbdepdfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_MBD_EPD' and filename like 'DST_MBD_EPD_$outfilelike-%' and runnumber = $runnumber");
#print "select filename,segment from datasets where dsttype = 'DST_MBD_EPD' and filename like 'DST_MBD_EPD_$outfilelike-%' and runnumber = $runnumber\n";
#die;
...
foreach my $segment (sort keys %trkhash)
{
	...
	my $subcmd = sprintf("perl run_condor.pl %d %s %s %s %s %s %s %d %d %s", $outevents, $pileup, $lfn, $mbdepdhash{sprintf("%06d",$segment)}, $outfilename, $outdir, $build, $runnumber, $segment, $tstflag);
	print "cmd: $subcmd\n";
	system($subcmd);
	my $exit_value  = $? >> 8;
	if ($exit_value != 0)

...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRACKSEEDS' and filename like 'DST_TRACKSEEDS_$outfilestring-%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
my $getclusterfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_CALO_CLUSTER' and filename like 'DST_CALO_CLUSTER_$outfilestring-%' and runnumber = $runnumber");
if ($verbosity > 0)
{
    print "select filename,segment from datasets where dsttype = 'DST_TRACKSEEDS' and filename like '%$outfilestring-%' and runnumber = $runnumber order by segment\n";
    print "select filename,segment from datasets where dsttype = 'DST_CALO_CLUSTER' and filename like '%$outfilestring-%' and runnumber = $runnumber\n";
}
...
foreach my $segment (sort keys %trkhash)
{
	...
	my $subcmd = sprintf("perl run_condor.pl %d %s %s %s %s %s %s %d %d %s", $outevents, $pileup, $lfn, $clusterhash{sprintf("%06d",$segment)}, $outfilename, $outdir, $build, $runnumber, $segment, $tstflag);
	print "cmd: $subcmd\n";
	system($subcmd);
	my $exit_value  = $? >> 8;
	if ($exit_value != 0)

...
my $executable = sprintf("%s/run_pass4_jobC_oo.sh",$rundir);
my $nevents = $ARGV[0];
my $pileup = $ARGV[1];
my $infile1 = $ARGV[2];
my $infile2 = $ARGV[3];
...
my $suffix = sprintf("_%010d-%06d",$runnumber,$sequence);
my $logdir = sprintf("%s/log/run%d/%s",$localdir,$runnumber,$pileup);
if (! -d $logdir)
{
  mkpath($logdir);
}
my $condorlogdir = sprintf("/tmp/OO_0_15fm/pass4_jobC/run%d/%s",$runnumber,$pileup);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}
...
print F "Arguments       = \"$nevents $pileup $infile1 $infile2 $dstoutfile $dstoutdir $build $runnumber $sequence\"\n";
...
print F "$executable, $nevents, $pileup, $infile1, $infile2, $dstoutfile, $dstoutdir, $build, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio, $memory $batchname\n";
close(F);

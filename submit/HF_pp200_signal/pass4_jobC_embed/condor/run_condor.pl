...
if ($#ARGV < 8)
{
    print "usage: run_condor.pl <events> <seeds infile> <cluster infile> <quarkfilter> <outfile> <outdir> <runnumber> <sequence> <fm range>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass4_jobC_embed_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass4_jobC_embed/%s/run%d/%s",$fm,$runnumber,$quarkfilter);
mkpath($condorlogdir);
...
print F "Arguments       = \"$nevents $infile1 $infile2 $dstoutfile $dstoutdir $quarkfilter $runnumber $sequence\"\n";
...
print F "$executable, $nevents, $infile1, $infile2, $dstoutfile, $dstoutdir, $quarkfilter, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

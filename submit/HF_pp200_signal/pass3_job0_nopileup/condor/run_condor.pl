...
my $executable = sprintf("%s/run_pass3_job0_nopileup_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass3_job0_nopileup/run%d/%s",$runnumber,$quarkfilter);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

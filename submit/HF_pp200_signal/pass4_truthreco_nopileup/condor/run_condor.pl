...
my $executable = sprintf("%s/run_pass4_truthreco_nopileup_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass4_truthreco_nopileup/run%d/%s",$runnumber,$quarkfilter);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

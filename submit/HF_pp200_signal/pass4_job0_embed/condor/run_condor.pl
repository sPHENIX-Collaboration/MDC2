...
if ($#ARGV < 6)
{
    print "usage: run_condor.pl <events> <quarkfilter> <infile> <outfile> <outdir> <runnumber> <sequence> <fm range>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass4_job0_embed_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass4_job0_embed/%s/run%d/%s",$fm,$runnumber,$quarkfilter);
mkpath($condorlogdir);

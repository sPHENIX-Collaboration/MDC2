...
if ($#ARGV < 5)
{
    print "usage: run_condor.pl <events> <quarkfilter> <truth infile> <outfile> <outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass3jet_nopileup_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass3jet_nopileup/%s",$quarkfilter);
mkpath($condorlogdir);

...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRUTH' and filename like '%pythia8_$quarkfilterWithDash%' and runnumber = $runnumber order by filename") || die $DBI::errstr;
...
while (my @res = $getfiles->fetchrow_array())
{
    ...
    if ($lfn =~ /(\S+)-(\d+)-(\d+).*\..*/ )
    {
        my $runnumber = int($2);
        my $segment = int($3);
        my $outfilename = sprintf("DST_TRUTH_JET_pythia8_%s-%010d-%05d.root",$quarkfilter,$runnumber,$segment);

...
if ($#ARGV < 7)
{
    print "# of args to low, saw $#ARGV, need 6\n";
    print "usage: run_condor.pl <events>  <quarkfilter> <g4hit infile> <outfile> <outdir> <runnumber> <sequence> <fm range>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass3calo_embed_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass3calo_embed/%s/run%d/%s",$fm,$runnumber,$quarkfilter);
mkpath($condorlogdir);

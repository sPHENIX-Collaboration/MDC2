...
my $executable = sprintf("%s/run_pass3_nopileup.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass3_nopileup");
mkpath($condorlogdir);
...
print F "Arguments       = \"$nevents $infile $dstoutfile $dstoutdir\"\n";

...
if ($#ARGV < 3)
{
    print "usage: run_condor.pl <events> <quarkfilter> <trkr g4hit file> <trkr cluster file> <tracks file> <truth file> <outfile> <outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass5_truthreco_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
my $quarkfilterMHz = sprintf("%s_3MHz",$quarkfilter);
my $condorlistfile = sprintf("condor.list");
my $suffix = sprintf("%s-%010d-%05d",$quarkfilterMHz,$runnumber,$sequence);
my $logdir = sprintf("%s/log/run%d/%s",$localdir,$runnumber,$quarkfilterMHz);
if (! -d $logdir)
{
  mkpath($logdir);
}
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass5_truthreco/run%d/%s",$runnumber,$quarkfilterMHz);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

...
foreach my $segment (sort { $a <=> $b } keys %mbdhash)
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_MBD_EPD_pythia8_$quarkfilter-%010d-%06d.root",$outrunnumber,$segment);
	if ($segment < 100000)
	{
	    $outfilename = sprintf("DST_MBD_EPD_pythia8_$quarkfilter-%010d-%05d.root",$outrunnumber,$segment);
	}

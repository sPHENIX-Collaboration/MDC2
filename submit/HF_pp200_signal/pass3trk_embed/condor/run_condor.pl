...
if ($#ARGV < 7)
{
    print "usage: run_condor.pl <events> <quarkfilter> <trk infile> <truth infile> <outdir> <runnumber> <sequence> <fm range>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass3trk_embed_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass3trk_embed/%s/run%d/%s",$fm,$runnumber,$quarkfilter);
mkpath($condorlogdir);
...
print F "Arguments       = \"$nevents $infile0 $infile1 $dstoutdir $quarkfilter $runnumber $sequence $fm\"\n";
...
print F "$executable, $nevents, $infile0, $infile1, $dstoutdir, $quarkfilter, $runnumber, $sequence, $fm, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

...
{
    print "second argument has to be either Charm, CharmD0, CharmD0piKJet5, CharmD0piKJet12, Bottom or BottomD0\n";
    exit(1);
}
...
my $getfiles = $dbh->prepare("select filename from datasets where dsttype = 'G4HitsOld' and filename like '%pythia8_$quarkfilterWithUnderScore%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $segment = int($3);
	my $foundall = 1;
	my $outfilename =  sprintf("G4Hits_pythia8_%s-%010d-%05d.root",$quarkfilter,$runnumber,$segment);

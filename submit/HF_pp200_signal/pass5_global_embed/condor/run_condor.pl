...
if ($#ARGV < 8)
{
    print "usage: run_condor.pl <events> <seeds infile> <bbcepd infile> <quarkfilter> <outfile> <outdir> <runnumber> <sequence> <fm range>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass5_global_embed_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass5_global_embed/%s/run%d/%s",$fm,$runnumber,$quarkfilter);
mkpath($condorlogdir);

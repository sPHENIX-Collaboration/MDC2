...
if ($#ARGV < 3)
{
    print "usage: run_condor.pl <events> <quarkfilter> <trk infile> <truth infile> <outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass3trk_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass3trk/run%d/%s",$runnumber,$quarkfilter);
mkpath($condorlogdir);
...
print F "Arguments       = \"$nevents $infile0 $infile1 $dstoutdir $quarkfilter $runnumber $sequence\"\n";
...
print F "$executable, $nevents, $infile0, $infile1, $dstoutdir, $quarkfilter, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

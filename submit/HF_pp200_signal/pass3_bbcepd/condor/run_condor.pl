...
my $executable = sprintf("%s/run_pass3_bbcepd_hf.sh",$rundir);
my $nevents = $ARGV[0];
...
my $suffix = sprintf("%s-%010d-%06d",$quarkfilter,$runnumber,$sequence);
if ($sequence < 100000)
{
    $suffix = sprintf("%s-%010d-%05d",$quarkfilter,$runnumber,$sequence);
}
my $logdir = sprintf("%s/log/run%d/%s",$localdir,$runnumber,$quarkfilter);
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/HF_pp200_signal/pass3_bbcepd/run%d/%s",$runnumber,$quarkfilter);
mkpath($condorlogdir);

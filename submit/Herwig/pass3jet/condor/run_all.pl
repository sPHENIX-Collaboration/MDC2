...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRUTH' and filename like 'DST_TRUTH_Herwig_$jettrigger-%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
if ($verbosity > 0)
{
    print "select filename,segment from datasets where dsttype = 'DST_TRUTH' and filename like 'DST_TRUTH_Herwig_$jettrigger-%' and runnumber = $runnumber order by segment\n";
}

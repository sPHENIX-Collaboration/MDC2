...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRKR_G4HIT' and filename like 'DST_TRKR_G4HIT_Herwig_$jettrigger-%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
my $chkfile = $dbh->prepare("select lfn from files where lfn=?") || die $DBI::errstr;
my $gettruthfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRUTH_G4HIT' and filename like 'DST_TRUTH_G4HIT_Herwig_$jettrigger-%' and runnumber = $runnumber");
if ($verbosity > 0)
{
  print "select filename,segment from datasets where dsttype = 'DST_TRKR_G4HIT' and filename like 'DST_TRKR_G4HIT_Herwig_$jettrigger-%' and runnumber = $runnumber order by segment\n";
  print "select filename,segment from datasets where dsttype = 'DST_TRUTH_G4HIT' and filename like 'DST_TRUTH_G4HIT_Herwig_$jettrigger-%' and runnumber = $runnumber\n";
}
my $nsubmit = 0;
...
foreach my $segment (sort { $a <=> $b } keys %trkhash)
{
	...
	{
            my $lfn =  sprintf("%s_Herwig_%s-%010d-%06d.root",$type,$jettrigger,$runnumber,$segment);
	    $chkfile->execute($lfn);
	    if ($chkfile->rows > 0)
	    {
		next;

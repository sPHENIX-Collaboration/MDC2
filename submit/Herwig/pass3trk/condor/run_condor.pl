...
my $executable = sprintf("%s/run_pass3trk_herwig.sh",$rundir);
my $nevents = $ARGV[0];
...
my $condorlogdir = sprintf("/tmp/Herwig/pass3trk/run%d/%s",$runnumber,$jettrigger);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}
...
print F "Arguments       = \"$nevents $infile0 $infile1 $dstoutdir $jettrigger $build $runnumber $sequence\"\n";
...
print F "$executable, $nevents, $infile0, $infile1, $dstoutdir, $jettrigger, $build, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio, $memory $batchname\n";
close(F);

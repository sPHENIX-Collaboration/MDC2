...
if ($jettrigger  ne "Jet10" &&
    ...
    $jettrigger  ne "MB")
{
    print "second argument has to be Jet10, Jet30, Jet40 or MB\n";
    exit(1);
}
...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRACKSEEDS' and filename like '%Herwig_$jettriggerWithDash%' and runnumber = $runnumber order by filename") || die $DBI::errstr;
my $getclusterfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_CALO_CLUSTER' and filename like '%Herwig_$jettriggerWithDash%' and runnumber = $runnumber");
...
foreach my $segment (sort keys %trkhash)
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_TRACKS_Herwig_%s-%010d-%06d.root",$jettrigger,$runnumber,$segment);

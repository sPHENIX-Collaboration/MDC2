...
if ($hostname !~ /phnxsub/)
{
    print "submit only from phnxsub01 to phnxsub04\n";
    exit(1);
}
...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $subcmd = sprintf("perl run_condor.pl %d %s %s %s %d %d %s %s", $outevents, $lfn, $outfilename, $outdir, $outrunnumber, $segment, $field, $tstflag);
	print "cmd: $subcmd\n";
	system($subcmd);
	my $exit_value  = $? >> 8;
	if ($exit_value != 0)

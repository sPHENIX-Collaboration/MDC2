...
if ($#ARGV < 11)
{
    print "usage: run_condor.pl <events> <particle> <ptmin> <ptmax> <trk embedfile> <bbc embedfile> <calo embedfile> <truth embedfile> <vertex embedfile> <outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/single_particle/pass2_embed/%s",$particle);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}
...
print F "$executable, $nevents, $particle, $ptmin, $ptmax, $infile0,  $infile1, $infile2, $infile3, $infile4, $dstoutdir, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

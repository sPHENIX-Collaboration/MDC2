...
my $shared;
my $condorprio = 55;
GetOptions("priority:i"=>\$condorprio, "test"=>\$test, "increment"=>\$incremental, "shared" => \$shared);
if ($#ARGV < 1)
{
...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $subcmd = sprintf("perl run_condor.pl %d %s %d %d %s %s %s %d %d %s", $outevents, $particle, $ptmin, $ptmax, $lfn, $outfilename, $outdir, $runnumber, $segment, $tstflag);
	print "cmd: $subcmd\n";
	system($subcmd);
	my $exit_value  = $? >> 8;
	if ($exit_value != 0)

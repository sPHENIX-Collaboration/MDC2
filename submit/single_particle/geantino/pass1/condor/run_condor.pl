...
my $executable = sprintf("%s/run_pass1_geantino.sh",$rundir);
my $nevents = $ARGV[0];
my $particle = $ARGV[1];
my $dstoutdir = $ARGV[2];
...
my $condorlogdir = sprintf("/tmp/single_particle/geantino/pass1/run%d",$runnumber);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}
...
print F "Arguments       = \"$nevents $particle $dstoutfile $dstoutdir $runnumber $sequence\"\n";
...
print F "$executable, $nevents, $particle, $dstoutfile, $dstoutdir, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

...
if ($#ARGV < 7)
{
    print "usage: run_condor.pl <events> <particle> <seeds infile> <cluster infile> <particle> <outfile> <outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/single_particle/upsilon/pass3_jobC_nopileup");
mkpath($condorlogdir);
...
print F "Arguments       = \"$nevents $infile1 $infile2 $dstoutfile $dstoutdir $particle $runnumber $sequence\"\n";
...
print F "$executable, $nevents, $infile1, $infile2, $dstoutfile, $dstoutdir, $particle, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

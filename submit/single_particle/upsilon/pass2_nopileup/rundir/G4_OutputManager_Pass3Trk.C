...
void CreateDstOutput(int runnumber, int segment, const string &particle)
{
  ...
  char segrun[100];
  snprintf(segrun,100,"%s-%010d-%05d",particle.c_str(),runnumber,segment);
  string FullOutFile = "DST_TRUTH_single_" + string(segrun) + ".root";;
  ...
  FullOutFile = "DST_TRKR_HIT_single_" + string(segrun) + ".root";;

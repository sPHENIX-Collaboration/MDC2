...
if ($#ARGV < 7)
{
    print "usage: run_condor.pl <events> <particle> <infile> <calo outfile>  <calo outdir> <trk outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/single_particle/upsilon/pass2_nopileup");
mkpath($condorlogdir);
...
print F "Arguments       = \"$nevents $infile $calooutfile $calodstoutdir $trkdstoutdir $particle\"\n";
...
print F "$executable, $nevents, $infile, $calooutfile $calodstoutdir $trkdstoutdir, $particle, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

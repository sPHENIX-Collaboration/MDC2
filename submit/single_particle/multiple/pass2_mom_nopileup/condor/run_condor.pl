...
if ($#ARGV < 3)
{
    print "usage: run_condor.pl <events> <filetype> <infile> <calo outfile>  <calo outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass2_mom_nopileup.sh",$rundir);
my $nevents = $ARGV[0];
my $particle = $ARGV[1];
my $filetype = $ARGV[2];
my $infile = $ARGV[3];
...
my $condorlogdir = sprintf("/tmp/single_particle/multiple/pass2_mom_nopileup/run%d/%s",$runnumber,$particle);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}
...
print F "Arguments       = \"$nevents $infile $calooutfile $calodstoutdir $runnumber $sequence $filetype\"\n";
...
print F "$executable, $nevents, $infile, $calooutfile, $calodstoutdir, $runnumber, $sequence, $filetype, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

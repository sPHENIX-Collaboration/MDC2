...
if ($#ARGV < 4)
{
    print "usage: run_condor.pl <nevents> <jettrigger> <infile> <outfile> <outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_cemc_geo_fix_js.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/JS_pp200_signal/cemc_geo_fix/cemc_hits/%s", $jettrigger);
mkpath($condorlogdir);
...
print F "Arguments       = \"$nevents $infile $dstoutfile $dstoutdir $runnumber $sequence $jettrigger\"\n";
...
print F "$executable, $nevents, $infile, $dstoutfile, $dstoutdir, $runnumber, $sequence, $jettrigger, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

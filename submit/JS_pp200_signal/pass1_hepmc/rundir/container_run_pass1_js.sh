...
echo running root.exe -q -b Fun4All_G4_JS_pp_HepMC.C\($1,\"$2\",\"$3\",\"$4\",\"$cdbtag\",\"$8\"\)
root.exe -q -b Fun4All_G4_JS_pp_HepMC.C\($1,\"$2\",\"$3\",\"$4\",\"$cdbtag\",\"$8\"\)

[[ -f copyscript.sh ]] && sh copyscript.sh

timedirname=/sphenix/sim/sim01/sphnxpro/mdc2/logs/js_pp200_signal/pass1_hepmc/timing.run${6}/${2}

...
my $getfiles = $dbh->prepare("select filename from datasets where dsttype = 'DST_HEPMC' and filename like 'DST_HEPMC_pythia8_$jettriggerWithUnderScore%' and runnumber = $runnumber order by segment") || die $DBI::errstr;
my $chkfile = $dbh->prepare("select lfn from files where lfn=?") || die $DBI::errstr;

my $getbkglastsegment = $dbh->prepare("select max(segment) from datasets where dsttype = 'DST_HEPMC' and filename like 'DST_HEPMC_pythia8_Detroit-%' and runnumber = $runnumber");
...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $outfilename = sprintf("DST_HEPMC_TRUTH_pythia8_%s-%010d-%06d.root",$jettrigger,$outrunnumber,$segment);
	...
	if ($foundall == 0) # background file(s) is missing
	{
#	    print "foundall is 1\n";
	    next;
	}
	...
	my $subcmd = sprintf("perl run_condor.pl %d %s %s %s %s %s %s %s %d %d %s", $outevents, $jettrigger, $lfn, $bkglistfile, $outfilename, $outdir, $build, $pileup, $runnumber, $segment, $tstflag);
	print "cmd: $subcmd\n";
       	system($subcmd);
	my $exit_value  = $? >> 8;
	if ($exit_value != 0)

...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_CALO_G4HIT' and filename like '%$outfilelike%' and runnumber = $inrunnumber order by segment") || die $DBI::errstr;
...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_CALO_NOZERO_%s-%010d-%06d.root",$outfilelike,$outrunnumber,$segment);
	if ($segment < 100000)
	{
	    $outfilename = sprintf("DST_CALO_NOZERO_%s-%010d-%05d.root",$outfilelike,$outrunnumber,$segment);
	}

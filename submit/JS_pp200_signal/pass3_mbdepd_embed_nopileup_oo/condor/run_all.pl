...
my $shared;
my $test;
GetOptions("build:s" => \$build, "increment"=>\$incremental, "fm:s" =>\$fm, "overwrite"=> \$overwrite, "run:i" =>\$runnumber, "shared" => \$shared, "test"=>\$test);
if ($#ARGV < 1)
{
...
my $outfilelike = sprintf("pythia8_%s_sHijing_OO_%s",$jettrigger,$fm);
...
foreach my $segment (sort keys %mbdhash)
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_MBD_EPD_%s-%010d-%06d.root",$outfilelike,$runnumber,$segment);
	...
	my $subcmd = sprintf("perl run_condor.pl %d %s %s %s %s %s %s %d %d %s %s", $outevents, $jettrigger, $lfn, $truthhash{sprintf("%06d",$segment)}, $outfilename, $outdir, $build, $runnumber, $segment,  $fm, $tstflag);
	print "cmd: $subcmd\n";
	system($subcmd);
	my $exit_value  = $? >> 8;
	if ($exit_value != 0)

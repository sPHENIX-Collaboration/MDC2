...
my $executable = sprintf("%s/run_pass3_mbdepd_embed_nopileup_oo_js.sh",$rundir);
my $nevents = $ARGV[0];
...
my $condorlogdir = sprintf("/tmp/JS_pp200_signal/pass3_mbdepd_embed_nopileup_oo/%s/run%d/%s",$fm,$runnumber,$jettrigger);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

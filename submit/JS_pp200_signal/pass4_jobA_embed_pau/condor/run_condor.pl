...
if ($#ARGV < 5)
{
    print "usage: run_condor.pl <events> <jettrigger> <outfile> <outdir> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass4_jobA_embed_pau_js.sh",$rundir);
my $nevents = $ARGV[0];
...
my $condorlogdir = sprintf("/tmp/JS_pp200_signal/pass4_jobA_embed_pau/run%d/%s",$runnumber,$jettrigger);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

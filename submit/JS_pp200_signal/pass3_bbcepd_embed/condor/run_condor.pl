...
if ($#ARGV < 8)
{
    print "usage: run_condor.pl <events> <jettrigger> <bbc infile> <truth infile> <outfile> <outdir> <runnumber> <sequence> <fm range>\n";
    print "options:\n";
...
my $condorlogdir = sprintf("/tmp/JS_pp200_signal/pass3_bbcepd_embed/%s/run%d/%s",$fm,$runnumber,$jettrigger);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

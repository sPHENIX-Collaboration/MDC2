...
my $executable = sprintf("%s/run_pass5_truthreco_embed_pau_js.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/JS_pp200_signal/pass5_truthreco_embed_pau/run%d/%s",$runnumber,$jettrigger);
mkpath($condorlogdir);

...
my $test;
my $memory = sprintf("5800MB"); # 50GeV jets observed < 5600MB
my $overwrite;
...
my $executable = sprintf("%s/run_pass2_embed_nopileup_oo_js.sh",$rundir);
my $nevents = $ARGV[0];
...
my $condorlogdir = sprintf("/tmp/JS_pp200_signal/pass2_embed_nopileup_oo/%s/run%d/%s",$fm,$runnumber,$jettrigger);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

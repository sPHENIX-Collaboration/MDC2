...
my $executable = sprintf("%s/run_pass3jet_embed_nopileup_js.sh",$rundir);
my $nevents = $ARGV[0];
...
my $condorlogdir = sprintf("/tmp/JS_pp200_signal/pass3jet_embed_nopileup/%s/run%d/%s",$fm,$runnumber,$jettrigger);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

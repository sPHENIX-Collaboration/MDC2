...
if ($#ARGV < 9)
{
    print "usage: run_condor.pl <events> <seeds infile> <cluster infile> <jettrigger> <outfile> <outdir> <build> <runnumber> <sequence> <fm range>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass4_jobC_embed_nopileup_js.sh",$rundir);
my $nevents = $ARGV[0];
...
my $condorlogdir = sprintf("/tmp/JS_pp200_signal/pass4_jobC_embed_nopileup/%s/run%d/%s",$fm,$runnumber,$jettrigger);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}
...
print F "Arguments       = \"$nevents $infile1 $infile2 $dstoutfile $dstoutdir $jettrigger $build $runnumber $sequence\"\n";
...
print F "$executable, $nevents, $infile1, $infile2, $dstoutfile, $dstoutdir, $jettrigger, $build, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio, $memory, $batchname\n";
close(F);

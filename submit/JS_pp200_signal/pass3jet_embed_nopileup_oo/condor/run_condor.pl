...
if ($#ARGV < 8)
{
    print "usage: run_condor.pl <events> <jettrigger> <truth infile> <outfile> <outdir> <build> <runnumber> <sequence> <fm range>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass3jet_embed_nopileup_oo_js.sh",$rundir);
my $nevents = $ARGV[0];
...
my $condorlogdir = sprintf("/tmp/JS_pp200_signal/pass3jet_embed_nopileup_oo/%s/run%d/%s",$fm,$runnumber,$jettrigger);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

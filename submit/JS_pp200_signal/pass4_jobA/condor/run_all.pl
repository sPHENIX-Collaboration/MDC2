...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRKR_CLUSTER' and filename like '%pythia8_$jettrigger-%' and runnumber = $runnumber order by segment") || die $DBI::errstr;

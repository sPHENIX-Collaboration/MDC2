...
my $executable = sprintf("%s/run_pass3_jobA_nopileup_js.sh",$rundir);
my $nevents = $ARGV[0];
...
mkpath($logdir);
my $condorlogdir = sprintf("/tmp/JS_pp200_signal/pass3_jobA_nopileup/run%d/%s",$runnumber,$jettrigger);
mkpath($condorlogdir);

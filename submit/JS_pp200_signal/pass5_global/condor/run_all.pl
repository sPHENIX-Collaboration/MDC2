...
my $phenix;
my $MHz = 3;
GetOptions("test"=>\$test, "increment"=>\$incremental, "MHz:i" => \$MHz,  "overwrite"=>\$overwrite,"phenix" => \$phenix, "shared" => \$shared);
if ($#ARGV < 1)
{
...
foreach my $segment (sort keys %trkhash)
{
	...
	my $subcmd = sprintf("perl run_condor.pl %d %s %s %s %s %s %d %d %s", $outevents, $jettrigger, $lfn, $mbdepdhash{sprintf("%06d",$segment)}, $outfilename, $outdir, $runnumber, $segment, $tstflag);
	print "cmd: $subcmd\n";
	system($subcmd);
	my $exit_value  = $? >> 8;
	if ($exit_value != 0)

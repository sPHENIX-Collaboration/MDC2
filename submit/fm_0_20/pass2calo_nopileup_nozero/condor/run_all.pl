...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_CALO_NOZERO_sHijing_0_20fm-%010d-%06d.root",$outrunnumber,$segment);
	if ($segment < 100000)
	{
	    $outfilename = sprintf("DST_CALO_NOZERO_sHijing_0_20fm-%010d-%05d.root",$outrunnumber,$segment);
	}

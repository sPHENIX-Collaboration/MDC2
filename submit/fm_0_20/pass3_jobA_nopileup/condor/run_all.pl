...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRKR_CLUSTER' and filename like 'DST_TRKR_CLUSTER_sHijing_0_20fm-%' and runnumber = $inrunnumber order by filename") || die $DBI::errstr;

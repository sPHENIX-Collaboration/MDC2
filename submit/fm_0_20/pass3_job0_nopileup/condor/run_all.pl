...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRKR_HIT' and filename like '%sHijing_0_20fm-%' and filename not like '%pythia8%' and runnumber = $runnumber order by filename") || die $DBI::errstr;
...
while (my @res = $getfiles->fetchrow_array())
{
	...
	my $runnumber = int($2);
	my $segment = int($3);
	my $outfilename = sprintf("DST_TRKR_CLUSTER_sHijing_0_20fm-%010d-%05d.root",$runnumber,$segment);

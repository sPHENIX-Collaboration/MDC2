...
my $executable = sprintf("%s/run_pass3_mbdepd_0_20fm.sh",$rundir);
my $nevents = $ARGV[0];
my $infile0 = $ARGV[1];
my $infile1 = $ARGV[2];
...
my $condorlogdir = sprintf("/tmp/fm_0_20/pass3_mbdepd/run%d",$runnumber);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}
...
print F "$executable, $nevents, $infile0 $infile1, $dstoutfile, $dstoutdir, $build, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio\n";
close(F);

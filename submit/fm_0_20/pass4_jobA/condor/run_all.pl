...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRKR_CLUSTER' and filename like 'DST_TRKR_CLUSTER_sHijing_0_20fm_50kHz_bkg_0_20fm%' and runnumber = $inrunnumber order by segment") || die $DBI::errstr;

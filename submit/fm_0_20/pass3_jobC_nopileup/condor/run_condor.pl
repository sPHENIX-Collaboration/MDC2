...
if ($#ARGV < 7)
{
    print "usage: run_condor.pl <events> <seeds infile> <cluster infile> <outfile> <outdir> <build> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass3_jobC_nopileup_fm_0_20.sh",$rundir);
my $nevents = $ARGV[0];
my $infile1 = $ARGV[1];
my $infile2 = $ARGV[2];
...
my $condorlogdir = sprintf("/tmp/fm_0_20/pass3_jobC_nopileup/run%d",$runnumber);
if (! -d $condorlogdir)
{
  mkpath($condorlogdir);
}

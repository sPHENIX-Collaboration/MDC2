...
my $getfiles = $dbh->prepare("select filename,segment from datasets where dsttype = 'DST_TRACKSEEDS' and filename like '%sHijing_0_20fm_50kHz_bkg_0_20fm%' and filename not like '%pythia8%' and runnumber = $inrunnumber order by filename") || die $DBI::errstr;

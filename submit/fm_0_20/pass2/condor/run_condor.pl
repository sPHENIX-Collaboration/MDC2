...
my $test;
my $memory = sprintf("9000MB");

GetOptions("memory:s"=>\$memory, "test"=>\$test);
if ($#ARGV < 5)
{
    print "usage: run_condor.pl <nevents> <infile> <outdir> <build> <runnumber> <sequence>\n";
    print "options:\n";
...
my $executable = sprintf("%s/run_pass2_0_20fm.sh",$rundir);
my $nevents = $ARGV[0];
my $infile = $ARGV[1];
...
print F "Arguments       = \"$nevents $infile $backgroundlist $dstoutdir $build $runnumber $sequence\"\n";
...
print F "$executable, $nevents, $infile, $backgroundlist, $dstoutdir, $build, $runnumber, $sequence, $outfile, $errfile, $condorlogfile, $rundir, $baseprio, $memory, $batchname\n";
close(F);

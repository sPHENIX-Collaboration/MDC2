...
use Getopt::Long;
use DBI;
# first physics run in run3pp: 53381
...
my $getruns = $dbh->prepare("select runnumber,segment,filename from datasets where runnumber >= $min_runnumber and runnumber <= $max_runnumber and filename like 'DST_CALOFITTING_run3pp_new_newcdbtag_v008-%' order by runnumber,segment");
...
while (my @runs = $getruns->fetchrow_array())
{
    ...
    my $outdir = sprintf("/sphenix/lustre01/sphnxpro/production2/run3pp/%s/calocalib/new_newcdbtag_v008",$runtype);
    my $qaoutdir = sprintf("/sphenix/data/data02/sphnxpro/production2/run3pp/%s/calocalib/new_newcdbtag_v008",$runtype);
    my $calib_ep_outdir = sprintf("/sphenix/data/data02/sphnxpro/production2/run3pp/%s/calibeventplane/new_newcdbtag_v008",$runtype);
    ...
    my $outfilename = sprintf("DST_CALO_run3pp_new_newcdbtag_v008-%08d-%05d.root",$runnumber,$segment);
    my $qaoutfilename = sprintf("HIST_CALOQA_run3pp_new_newcdbtag_v008-%08d-%05d.root",$runnumber,$segment);
    my $calib_ep_outfilename = sprintf("CALIB_EVENTPLANE_run3pp_new_newcdbtag_v008-%08d-%05d.root",$runnumber,$segment);
    if ($runs[2] =~ /beam/)
    {

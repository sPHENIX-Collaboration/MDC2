...
use Getopt::Long;
use DBI;
# first physics run in run3oo: 53881
# last physics run (from prod): 56000
my $outdir = sprintf("/sphenix/lustre01/sphnxpro/production2/run3oo/physics/ana533_nocdbtag_v001");
my $test;
my $incremental;
my $killexist;
my $overwrite;
my $shared;
my $events = 0;
GetOptions("increment"=>\$incremental, "events:i" => \$events, "killexist" => \$killexist, "overwrite" => \$overwrite, "shared" => \$shared, "test"=>\$test);
if ($#ARGV < 1)
{
    print "usage: run_runrange.pl <min runnumber> <max runnumber>\n";
    print "parameters:\n";
    print "--increment : submit jobs while processing running\n";
    print "--events: <number of events>, default = 10000\n";
...
my $getruns = $dbh->prepare("select runnumber from run where runnumber>= $min_runnumber and runnumber <= $max_runnumber and (runtype='physics' or runtype='beam') and eventsinrun >= 100000 and EXTRACT(EPOCH FROM (ertimestamp-brtimestamp)) >= 300 order by runnumber");

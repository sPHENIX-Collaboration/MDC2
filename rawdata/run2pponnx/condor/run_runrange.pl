...
my $qaoutdir = sprintf("/sphenix/data/data02/sphnxpro/production2/run2pp/physics/caloyonnxfitting/new_newcdbtag_v007");
...
while (my @runs = $getruns->fetchrow_array())
{
	...
	next;
       ...
       my $outfilename = sprintf("DST_CALOFITTING_ONNX_run2pp_new_newcdbtag_v007-%08d-%05d.root",$runnumber,$segment);
        my $qaoutfilename = sprintf("HIST_CALOFITTINGQA_ONNX_run2pp_new_newcdbtag_v007-%08d-%05d.root",$runnumber,$segment);

...
my $executable = sprintf("%s/run2pp_jetskim.sh",$rundir);
my $nevents = $ARGV[0];
my $runnumber = $ARGV[1];
my $segment = $ARGV[2];
my $infile = $ARGV[3];
...
my $condorlogdir = sprintf("/tmp/rawdata/run2ppjetskim");
if (! -d $condorlogdir)
{
    mkpath($condorlogdir);
}
...
print F "Arguments       = \"$nevents $runnumber $segment $infile $dstoutfile $dstoutdir $qafile $qadir $dstout2file\"\n";

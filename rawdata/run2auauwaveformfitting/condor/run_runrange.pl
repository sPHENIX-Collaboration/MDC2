...
my $outdir = sprintf("/sphenix/lustre01/sphnxpro/production2/run2auau/physics/caloy2fitting/new_newcdbtag_v006");
my $qaoutdir = sprintf("/sphenix/data/data02/sphnxpro/production2/run2auau/physics/caloy2fitting/new_newcdbtag_v006");
my $events = 0;
my $incremental;
my $killexist;
my $overwrite;
...
my $getruns = $dbh->prepare("select runnumber,segment from datasets where runnumber >= $min_runnumber and runnumber <= $max_runnumber and filename like 'DST_TRIGGERED_EVENT_seb18_run2auau_new_nocdbtag_v006-%' order by runnumber,segment");
...
while (my @runs = $getruns->fetchrow_array())
{
    my $runnumber=$runs[0];
    my $segment = $runs[1];
    my $typelike = sprintf("DST_TRIGGERED_EVENT_\%%_run2auau_new_nocdbtag_v006-\%%");
    ...
    my $outfilename = sprintf("DST_CALOFITTING_run2auau_new_newcdbtag_v006-%08d-%05d.root",$runnumber,$segment);
    my $qaoutfilename = sprintf("HIST_CALOFITTINGQA_run2auau_new_newcdbtag_v006-%08d-%05d.root",$runnumber,$segment);

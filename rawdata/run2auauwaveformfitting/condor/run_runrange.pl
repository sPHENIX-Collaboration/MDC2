...
my $getruns = $dbh->prepare("select runnumber,segment from datasets where runnumber >= $min_runnumber and runnumber <= $max_runnumber and filename like 'DST_TRIGGERED_EVENT_seb18_run2auau_ana502_nocdbtag_v001-%' order by runnumber,segment");
...
while (my @runs = $getruns->fetchrow_array())
{
    my $runnumber=$runs[0];
    my $segment = $runs[1];
    my $typelike = sprintf("DST_TRIGGERED_EVENT_\%%_run2auau_ana502_nocdbtag_v001-\%%");

...
#    }
    ...
    my $outdir = sprintf("/sphenix/lustre01/sphnxpro/production2/run3pp/%s/calofitting/new_newcdbtag_v008",$runtype);
    my $qaoutdir = sprintf("/sphenix/data/data02/sphnxpro/production2/run3pp/%s/calofitting/new_newcdbtag_v008",$runtype);
    ...
    my $outfilename = sprintf("DST_CALOFITTING_run3pp_new_newcdbtag_v008-%08d-%05d.root",$runnumber,$segment);
    my $qaoutfilename = sprintf("HIST_CALOFITTINGQA_run3pp_new_newcdbtag_v008-%08d-%05d.root",$runnumber,$segment);
    if ($runs[2] =~ /beam/)
    {

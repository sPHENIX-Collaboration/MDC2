...
my $outdir = sprintf("/sphenix/lustre01/sphnxpro/production2/run2pp/physics/caloonnxfitting/new_newcdbtag_v007");
my $qaoutdir = sprintf("/sphenix/data/data02/sphnxpro/production2/run2pp/physics/caloonnx/new_newcdbtag_v007");
my $events = 0;
my $incremental;
my $killexist;
my $overwrite;
...
my $getruns = $dbh->prepare("select runnumber,segment,filename from datasets where runnumber >= $min_runnumber and runnumber <= $max_runnumber and filename like 'DST_CALOFITTING_ONNX_run2pp_new_newcdbtag_v007-%' order by runnumber,segment");
...
while (my @runs = $getruns->fetchrow_array())
{
    ...
    my $segment = $runs[1];
    my $lfn = $runs[2];
    my $outfilename = sprintf("DST_CALO_ONNX_run2pp_new_newcdbtag_v007-%08d-%05d.root",$runnumber,$segment);
    my $qaoutfilename = sprintf("HIST_CALOQA_ONNX_run2pp_new_newcdbtag_v007-%08d-%05d.root",$runnumber,$segment);

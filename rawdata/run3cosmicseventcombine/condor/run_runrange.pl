...
my $outdir = sprintf("/sphenix/lustre01/sphnxpro/production2/run3cosmics/physics/ana527_nocdbtag_v001");
my $test;
my $incremental;
my $killexist;
my $shared;
...
my $getruns = $dbh->prepare("select runnumber from run where runnumber>= $min_runnumber and runnumber <= $max_runnumber and runtype='cosmics' and eventsinrun >= 100000 order by runnumber");
my $gethosts = $dbh->prepare("select hostname from hostinfo where runnumber = ? and (hostname = 'seb16' or hostname = 'seb17')");
my $nsubmit = 0;
$getruns->execute();

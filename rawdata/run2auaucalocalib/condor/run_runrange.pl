...
my $outdir = sprintf("/sphenix/lustre01/sphnxpro/production2/run2auau/physics/calocalib/ana509_2024p022_v001");
my $qaoutdir = sprintf("/sphenix/data/data02/sphnxpro/production2/run2auau/physics/calocalib/ana509_2024p022_v001");
my $events = 0;
my $incremental;
my $killexist;
my $overwrite;
...
my $getruns = $dbh->prepare("select runnumber,segment,filename from datasets where runnumber >= $min_runnumber and runnumber <= $max_runnumber and filename like 'DST_CALOFITTING_run2auau_ana509_2024p022_v001-%' order by runnumber,segment");
...
while (my @runs = $getruns->fetchrow_array())
{
    ...
    my $segment = $runs[1];
    my $lfn = $runs[2];
    my $outfilename = sprintf("DST_CALO_run2auau_ana509_2024p022_v001-%08d-%05d.root",$runnumber,$segment);
    my $qaoutfilename = sprintf("HIST_CALOQA_run2auau_ana509_2024p022_v001-%08d-%05d.root",$runnumber,$segment);
